...
> 02LMM 项目说明  
> 本仓库是一个基于 MiniMind 的深度探索项目，命名为 **02LMM (Zero to Large Multimodal Model)**。  
> 为什么叫 02LMM？因为在 DeepSeek-V3/R1、GPT-4o 等原生多模态模型成为标配的时代，我们不应只满足于 LLM，而是要从 0 开始，一步步构建出能看、能听、能说的**多模态大模型 (Large Multimodal Model)**。  
> 相关笔记与说明文档统一存放于 [Doc](./Doc/) 目录，内容基于 Vibe Writing 大模型输出结果整理与校订，并尽量与本仓库代码结构（model/、trainer/、scripts/ 等）保持联动。
